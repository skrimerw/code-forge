...
export async function runCheckList(id: number) {
  ...
  if (nonEmptyModules.size === modules.length && modules.length !== 0) {
  ...
  if (nonEmptyThemes.size === themes.length && themes.length !== 0) {
  ...
  const emptyCodeTask = codeTasks.find(
    (task) =>
      ...
      task.variants.find(
        (variant) =>
          variant.test.trim() === "" || variant.starterCode.trim() === ""
      )
  );
  ...
  if (!noLogoTheme && themes.length > 0) {
