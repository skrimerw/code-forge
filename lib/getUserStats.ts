...
export async function getUserStats(userId: number) {
  ...
  Array.from({ length: daysInMonth }).forEach((_, i) => (monthObj[i + 1] = 0));
    ...
    for (const codeSolution of solutionsArr) {
      ...
      if (year === currentYear) {
        codeSolutionsActivityData.year[monthMap[month]] =
          codeSolutionsActivityData.year[monthMap[month]] === undefined
            ? 1
        ...
        if (month === currentMonth) {
          ...
          if (
            ...
            codeSolutionsActivityData.week[weekDayMap[normalizedWeekDay]] =
              codeSolutionsActivityData.week[weekDayMap[normalizedWeekDay]] ===
              undefined
                ? 1
